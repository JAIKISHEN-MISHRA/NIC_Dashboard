...
exports.getDashboardData = async (req, res) => {
  ...
  if (division_code) {
    conditions.push(`division_code = $${idx++}`);
    values.push(division_code);
  }
  if (district_code) {
    conditions.push(`district_code = $${idx++}`);
    values.push(district_code);
  }
  if (taluka_code) {
    conditions.push(`taluka_code = $${idx++}`);
  ...
  try {
    ...
    // Merge all JSONB rows into one nested structure
    let merged = {};

    rows.forEach(row => {
      mergeDeepSum(merged, row.data);
    });
...
exports.getTimeSeriesData = async (req, res) => {
  ...
  if (state_code) {
    filters.push(`state_code = $${idx++}`);
    values.push(state_code);
  }
  if (division_code) {
    filters.push(`division_code = $${idx++}`);
    values.push(division_code);
  }
  if (district_code) {
    filters.push(`district_code = $${idx++}`);
    values.push(district_code);
  }
  if (taluka_code) {
    filters.push(`taluka_code = $${idx++}`);
    values.push(taluka_code);
  }
  ...
  try {
    ...
    result.rows.forEach(({ year, month, data }) => {
      ...
      mergeDeepSumTime(grouped[key].data, data); // merge data json
    });
