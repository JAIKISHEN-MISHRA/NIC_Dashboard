...
import { Box, Typography, Grid, Skeleton, Select, MenuItem, InputLabel, FormControl } from "@mui/material";
...
export default function AdminDashboard() {
  ...
  // cascading fetches (respect localStorage fixed flags)
  useEffect(() => {
    if (!selectedState) return;
    getDivisions(selectedState).then((res) => {
      setDivisions(res.data || []);
      if (!localStorage.getItem("division_code")) setSelectedDivision("");
      setDistricts([]);
      setTalukas([]);
    });
  }, [selectedState]);
  ...
  useEffect(() => {
    ...
    getDistricts(selectedState, selectedDivision).then((res) => {
      ...
      if (!localStorage.getItem("district_code")) setSelectedDistrict("");
      setTalukas([]);
    });
  ...
  useEffect(() => {
    ...
    getTalukas(selectedState, selectedDivision, selectedDistrict).then((res) => {
      setTalukas(res.data || []);
      if (!localStorage.getItem("taluka_code")) setSelectedTaluka("");
    });
  }, [selectedDistrict, selectedDivision, selectedState]);

  // When scheme changes, fetch its structure to get frequency
  useEffect(() => {
    if (!selectedScheme) {
  ...
  // --- Period helpers (same logic as Upload) ---
  ...
  // compute periods when relevant inputs change
  useEffect(() => {
    if (schemeFrequency === "Daily") {
      ...
      // default start/end to full month if not provided
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // require periods for frequency types that need them
    ...
    try {
      const payload = {
        ...
        periods, // array (eg daily dates or weekly labels or months/years)
      };
  ...
  // small UI for period selection based on frequency
  ...
  return (
    <Box p={3}>
      ...
      {/* Show the current frequency and the period controls */}
      <Box mt={2}>
